...
fn test_refs() {
  let x: &u32 = &42;
  let y: &[u8; 4] = x.cast_in_place().unwrap();
...
fn test_option() {
  ...
  let y: Option<&[u8; 4]> = x.cast_in_place().unwrap();
...
fn test_vec() {
  ...
  let y: Result<Vec<u8>, _> = x.cast_in_place();
  y.unwrap_err();

  let x: Vec<bool> = vec![true, false, true, false];
  let y: Vec<u8> = x.cast_in_place().unwrap();
  assert_eq!(*y, [1, 0, 1, 0]);
}
...
fn test_box_slice() {
  ...
  let y: Result<Box<[u8]>, _> = x.cast_in_place();
  y.unwrap_err();

  let x: Box<[bool]> = vec![true, false, true, false].into_boxed_slice();
  let y: Box<[u8]> = x.cast_in_place().unwrap();
  assert_eq!(*y, [1, 0, 1, 0]);
}
...
fn test_rc() {
  ...
  let y: Result<Rc<[u8; 4]>, _> = x.cast_in_place();
  y.unwrap_err();

  let x: Rc<[u32]> = vec![42].into();
  let y: Result<Rc<[u8]>, _> = x.cast_in_place();
  y.unwrap_err();

  let x: Rc<[bool]> = vec![true, false, true, false].into();
  let y: Rc<[u8]> = x.cast_in_place().unwrap();
  assert_eq!(*y, [1, 0, 1, 0]);
...
fn test_arc() {
  ...
  let y: Result<Arc<[u8]>, _> = x.cast_in_place();
  y.unwrap_err();
